...
templates.html                  原模板下载页（自动生成，四个模板各一个下载按钮）
...
首页点 **「下载原模板（Word，共 4 个）」** → 进入 `templates.html`，四个文件各有一个「下载」按钮：
...
模板文件由 `publish.ps1` 每次发布时从工作区 `D:\雄安\` 自动复制进 `versions/<版本>/`，
所以线上拿到的就是你给的那四个原文件（已做过 SHA-256 字节比对，完全一致）。
